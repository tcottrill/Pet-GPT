...
// cpu_6502.cpp
// Reverted back the stack operations and reset to use the memory handlers. This slows things down but is 100% needed for some games.
// I left int he old code commented out for reference.
...
void cpu_6502::push16(uint16_t val)
{
	...
	//MEM[BASE_STACK + S] = (uint8_t)((val >> 8) & 0xFF);
	//MEM[BASE_STACK + ((S - 1) & 0xFF)] = (uint8_t)(val & 0xFF);
	S -= 2;
}
...
uint16_t cpu_6502::pull16()
{
	...
	//uint16_t val = MEM[BASE_STACK + ((S + 1) & 0xFF)] |
	//((uint16_t)MEM[BASE_STACK + ((S + 2) & 0xFF)] << 8);
	S += 2;
	return val;
}
...
int cpu_6502::step6502()
{
	...
	debug =0;
	//LOG_INFO("IRQ PENDING: %d", _irqPending);
	if (debug) {
		int bytes = 0;
